param("app_name", description="The name for the app", default="FastHTML App")
...
param("dev_command",
      ...
      default="""export PATH=/app/.venv/bin:$PATH; if [ -f uv.lock ]; then uv sync --locked; elif [ -f requirements.txt ]; then [ -d .venv ] || uv venv .venv; uv pip install -q -r requirements.txt uvicorn; fi; if [ "${WORKERS:-1}" = "1" ]; then exec uvicorn --root-path "$CL_APP_PATH" --reload --host 0.0.0.0 --port 5000 ${APP_MODULE:-app:app}; else exec uvicorn --root-path "$CL_APP_PATH" --host 0.0.0.0 --port 5000 --workers ${WORKERS} ${APP_MODULE:-app:app}; fi""")
